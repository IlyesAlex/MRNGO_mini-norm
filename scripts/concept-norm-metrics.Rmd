...
## READ-IN KREMER_MERGED

```{r}
kremer_merged_data = read_excel("../others/Kremer_norm_filtered-merged.xlsx",
                 sheet = "Kremer_norm_filtered-merged")
kremer_merged_cleaned = kremer_merged_data %>%
  mutate(Concept=as.factor(Concept)) %>% 
  mutate(Feature=as.factor(Feature)) %>% 
  mutate(ConceptClass=as.factor(ConceptClass)) %>% 
  mutate(`Concept(IT)`=as.factor(`Concept(IT)`)) %>%
  mutate(S_hat=as.numeric(S_hat)) %>% 
  mutate(sd_S_hat=as.numeric(sd_S_hat))
```
...
```{r}
kremer_merged_richness = kremer_merged_cleaned %>% 
  mutate(Dataset = "KREMER") %>% 
  select(Concept, natural, S_hat, Nationality, Dataset) %>% 
  distinct()

mrngo_data_clened_metrics_richness_filtered <- mrngo_data_clened_metrics_richness %>%
  mutate(Nationality="HUN")  %>% 
  mutate(Dataset = "MRNGO") %>% 
  select(Concept, natural, S_hat, Nationality, Dataset) %>% 
  distinct() %>% 
  filter(Concept %in%  kremer_merged_richness$Concept)

mrngo_kremer_merged <- bind_rows(kremer_merged_richness, mrngo_data_clened_metrics_richness_filtered)
...
mutate-tel a WB-s oszlopokat faktorrá kell alakítani, ifelse változóval olyan oszlop, amiben a WB_Maj alapján kategorizálom hogy fankcionális-e vagy sem (0 ha nem, 1 ha igen) summarise-olni kell a dataset-et úgy, asja meg, hogy azonosítónként hány százalékban van funkcionális label hogy hány fuknciónális label van (percent legyen a név mondjuk)
...
```{r}
t.test(percent_h1, above_seven, data=mrngo_cleaned_h1)
```

### H2

utánanézni, hogy hogyan lehet egy "or" logikai tesztet csinálni az ifelse parancsban (WB_Maj == "c" vagy "e") summarise-olni kell a dataset-et úgy, hogy számolja meg dataset-enként és conceptenként, hogy százalékban van 1-es label (percent legyen a név mondjuk)
...
német diáko
```{r}
mrngo_data_h4 = mrngo_kremer_merged %>% 
  select(Concept, natural, S_hat, Nationality, Dataset) %>% 
  distinct() %>% 
  filter(natural==1)

t.test(S_hat ~ Dataset, data=mrngo_data_h4)
```
